...
# Assume 'data' is your dataset with columns 'text' and 'label'
...
mlp_spec <- mlp(
  hidden_units = 64,         # one hidden layers with 64 and 32 units
...
test_metrics <- metric_set(accuracy, f_meas, roc_auc, npv, ppv, sensitivity, specificity)
mlp_test_pred %>% test_metrics(truth = sentiment, estimate = .pred_class, .pred_Negative:.pred_Positive)
